...
or Math.pow(x1-x2, 2) + Math.pow(y1-y2, 2)
return squareDistance <= ((r1+r2) * (r1+r2))
or Math.pow(r1+r2, 2)

((r1+r2) * (r1+r2)) is perhaps faster than Math.pow(r1+r2, 2) ... 
...
function nextLevel(xIsMoreThan, xIsLessThan, yIsLessThan, textAlert, xTo, yTo, newLevel, newPosX, newPosY) {
  if (x > xIsMoreThan && x < xIsLessThan && y < yIsLessThan) {
    alert(textAlert);
    /*
    var myWindow = window.open("", "", "width=300, height=100, top=400, left=${document.documentElement.clientWidth} / 2");
    myWindow.document.write(textAlert);
...
function paint(event) {
  ...
  nextLevel(canvasW - 2.4 * ballRadius - 2, canvasW - 2.4 * ballRadius + 2, 1.05 * ballRadius + 2, 
    'Congratulations! \nYou\'ve made it to level 2!', 2, 10, 2, canvasW / 2, canvasH - 2 * ballRadius - 1);
  if (level == 2) {paint2(event);};
  nextLevel(canvasW - 2.4 * ballRadius - 2, canvasW - 2.4 * ballRadius + 2, 1.05 * ballRadius + 2, 
    'Congratulations! \nYou\'ve made it to level 3!', 2, 10, 3, canvasW / 2, canvasH - 2 * ballRadius - 1);
  if (level == 3) {paint3(event);};
  nextLevel(canvasW - 2.4 * ballRadius - 2, canvasW - 2.4 * ballRadius + 2, 1.05 * ballRadius + 2, 
    'Congratulations! \nYou\'ve made it to the end!', 2, 10, 1, canvasW / 2, canvasH - 2 * ballRadius - 1);  
  console.log(level)
};
