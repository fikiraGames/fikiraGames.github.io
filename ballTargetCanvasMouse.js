...
//var y          = ballRadius * 2 - ballRadius - 1;
...
function moveTo(x, y, width, height, color) {
  ctx.beginPath();
  ctx.moveTo(x, y);
...
//moveTo(0, 23*ballRadius, 5*ballRadius, 25*ballRadius, '#FFFF00');
...
/*   x,  y = position middle of ball; cX, cY = "client" = mouse; px, py = position x, y + px  */
  ...
  var cX = event.clientX - ((body.clientWidth - canvas.width) / 2);
...
// very hard bounce
  ...
  */
  ...
  if (x > 1.1*canvas.width - ballRadius && y < 2.5*ballRadius) {
